<template>
  <main>

    <section class="container">
      <img
        class="profile-img"
        src="./assets/profile.jpg"
        alt="Rubin Lukongo Profile"
      />

      <h2>About Me</h2>
      <p>
        Hello! I am Rubin Lukongo, currently working at AECOM as part of the French
        Service Delivery team. I am passionate about delivering high-quality services,
        improving processes, and enhancing customer experience.
      </p>
    </section>

    <section class="container">
      <h2>Skills</h2>
      <ul>
        <li>French Service Delivery</li>
        <li>Process Improvement & Optimization</li>
        <li>Client Relationship Management</li>
        <li>Communication & Team Collaboration</li>
        <li>Basic HTML & CSS</li>
      </ul>
    </section>

    <section class="container contact">
      <h2>Contact</h2>
      <p>Email: <a href="mailto:shamanyerrubin@gmail.com">shamanyerrubin@gmail.com</a></p>
      <p>LinkedIn: <a href="https://www.linkedin.com/in/yourprofile" target="_blank">linkedin.com/in/yourprofile</a></p>
      <p>GitHub: <a href="https://github.com/yourprofile" target="_blank">github.com/yourprofile</a></p>
    </section>

  </main>
</template>

<script>
export default {
  name: "App"
};
</script>


style scoped>
/* Paste the contents of personalprofile.css here */
</style>

